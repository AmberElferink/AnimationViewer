	...
	def GetHeader(self,fullPath):
		if 'header' in self.c3d: 
			...
			if 'points' in self.c3d['header']: 													# if points exist
				self.Gen['NumbItems'] = self.c3d['header']['points']['size']                 # get number items (markers, angles, moments, etc.)
			...
			if 'analogs' in self.c3d['header']: 	                                            # if analogs exist
				self.Gen['Analog_FirstFrame'] = self.c3d['header']['analogs']['first_frame'] # get the analogs first frame
				...
				self.Gen['Analog_NumbChan'] = self.c3d['header']['analogs']['size']          # get the analogs number of channels 
	...
	def GetSubjects(self):
		if 'SUBJECTS' in self.c3d['parameters']: 
			if self.c3d['parameters']['SUBJECTS']['USED']['value'][0]>0:                               # if exist data
				...
				self.Gen['SubjName']  = self.c3d['parameters']['SUBJECTS']['NAMES']['value']        # get the name of the subject

	def GetPoint(self):
		if 'POINT' in self.c3d['parameters']: 
			if self.c3d['parameters']['POINT']['USED']['value'][0]>0: 							       # if exist data
				self.Labels['PointsName']= self.c3d['parameters']['POINT']['LABELS']['value']          # get all the labels (markers, angles, etc.)
				self.Gen['PointsUnit'] = self.c3d['parameters']['POINT']['UNITS']['value'][0]       # get the units of the coodinates  
				...
				if 'TYPE_GROUPS' in self.c3d['parameters']['POINT']:  					        # determine if groups exist (model outputs)
					...
					for k in range(0,len(GroupsInC3D)):
						...
						self.Data[GroupsInC3D[k].title()]  = self.GetData_Groups(MyList)     # store the data for each group available in c3d

						ExtList.extend(MyList) 													# create a temp list of all labels in groups
						self.GetUnits(GroupsInC3D[k]) 											# get the units of each group

				self.Labels['Markers']= sorted(list(set(self.Labels['PointsName']) ^ set(ExtList)))    # add only the labels for markers 
	...
	def GetForcePlatForm(self):
		if 'FORCE_PLATFORM' in self.c3d['parameters']: 
			if self.c3d['parameters']['FORCE_PLATFORM']['USED']['value'][0]>0:                                # if exist data
 				self.Gen['ForcePlateOrigin']= self.c3d['parameters']['FORCE_PLATFORM']['ORIGIN']['value']  # get the origin of force plate

	def GetEventContext(self):
		if 'EVENT_CONTEXT' in self.c3d['parameters']: 
			if self.c3d['parameters']['EVENT_CONTEXT']['USED']['value'][0]>0:                           # if exist data
 				self.Labels['EventLabels']= self.c3d['parameters']['EVENT_CONTEXT']['LABELS']['value']  # get the origin of force plate

	def GetAnalysis(self):
		if 'ANALYSIS' in self.c3d['parameters']: 
			if self.c3d['parameters']['ANALYSIS']['USED']['value'][0]>0:                           # if exist data
 				self.Labels['AnalysisNames']= self.c3d['parameters']['ANALYSIS']['NAMES']['value']  # get the origin of force plate

	def GetProcessing(self):
		if 'PROCESSING' in self.c3d['parameters']:
			self.Gen['SubjHeight']  = self.c3d['parameters']['PROCESSING']['Height']['value']   # get the height of the subject
			self.Gen['SubjMass']  = self.c3d['parameters']['PROCESSING']['Bodymass']['value']   # get the mass of the subject
			self.Gen['SubjLLegLength']  = self.c3d['parameters']['PROCESSING']['LLegLength']['value']   # get the LLegLenght of the subject
			self.Gen['SubjRLegLength']  = self.c3d['parameters']['PROCESSING']['RLegLength']['value']  # get the RLegLenght of the subject
	...
	def GetUnits(self, GroupsInC3D):            
		...
		for kk in range(0,len(GroupsUnit)):
			if GroupsUnit[kk].lower().find(GroupsInC3D.lower(),0, len(GroupsInC3D)) != -1:
				self.Gen[GroupsUnit[kk]]= self.c3d['parameters']['POINT'][Units[kk]]['value'][0]   # add the group units to the header 
				break
